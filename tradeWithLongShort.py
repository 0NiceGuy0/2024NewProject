...
class AutoTrader:
    ...
    def trade_logic(self):
        ...
        if self.long_target is not None and self.short_target is not None:
            ...
            if self.reflash > 0 and self.reflash < 10000:
                df = pyupbit.get_ohlcv(self.ticker, interval=self.interval, count=1)
                current_candle_high = df.iloc[0]['high']
                check_stop_benefit = current_price < current_candle_high *0.999 #고가 대비 최소 0.1% 이상 떨어짐.
                    

            if current_price < self.short_target or check_stop_benefit or self.check_stop_loss(current_price):                
